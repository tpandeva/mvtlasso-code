...
#from model._tlasso import em_tlasso
...
#from model.utils import MSIICA, whiten, find_ordering
from sklearn.preprocessing import scale
import torch.nn as nn
import geotorch
from model._tlasso import run_glasso,em_tlasso,em_tlasso_noise_no_noise
...
def fit_ica_tlasso(Xs, ks, device, T, l,r, to_whiten, seed=1, params=None, scaling=True):
    ...
    for t in range(T):   
        ...
        if L_t is None: 
            ...
            S_all = [Xws[i] for i in range(num_views)]
            ...
            L_t, tauZ = em_tlasso_noise_no_noise(S_all, 1, l,r, L_t, glasso=True)
            ...
            Tau = [np.diag(tauZ[i]) for i in range(num_views)]
        ...
        for i in range(num_views): 
            ...
            for _ in range(2):
                ...
                # S1 = cp.Variable((n,n), "S1",PSD=True)
                # S2 = cp.Variable((n,n), "S2",PSD=True)
                D1 = np.sqrt(Ik[i])
                if Tau is not None:
                ...
                def loss_closure():
                    ...
                    loss = torch.trace(D1_tensor@(ZQ.T@(L_t_tensor@ZQ))@D1_tensor)
        ...
        L_t, tauZ = em_tlasso_noise_no_noise(Z_np, 1, l,r, L_t)
   ...
   # params = [50]
    ...
    for s in range(cfg.start,cfg.end):
        try:
            ...
            # for i in range(2): adj_Wi[i][adj_Wi[i]!=0]=1
