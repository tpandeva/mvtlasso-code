...
import geotorch
from model._tlasso import run_glasso,em_tlasso,em_tlasso_noise3
...
def fit_ica_tlasso(Xs, ks, device, T, l,r, to_whiten, seed=1, params=None, scale=True):
    np.random.seed(seed)
    num_views = len(ks)
    ...
    for i in range(num_views):
        ...
        models.append(TICA(Xica.shape[1], Xica.shape[1], scale).to(device))
    ...
    for t in range(T):   
        ...
        if L_t is None: 
            ...
            L_t, D_t, tauZ, tauN = em_tlasso_noise3(S_all, Z_all, 1, l,r, L_t, D_t, glasso=True)
        ...
        for i in range(num_views): 
            ...
            for _ in range(2):
                ...
                def loss_closure():
                    ...
                    loss = torch.trace(D1_tensor@(ZQ.T@(L_t_tensor@ZQ))@D1_tensor)+torch.trace(D2_tensor@(ZQ.T@(D_t_tensor@ZQ))@D2_tensor)#-torch.linalg.slogdet(models[i].W.weight*torch.eye(Z.shape[1]))[1]
        ...
        L_t, D_t, tauZ, tauN = em_tlasso_noise3(Z_np, N_np, 1, l,r, L_t, D_t)
...
def main(cfg) -> None:
   ...
   # params = [50]
    ...
    for s in range(cfg.start,cfg.end):
        try:
            Wi=fit_ica_tlasso(Xs, ks, device, T, l, r, to_whiten, s, params)
