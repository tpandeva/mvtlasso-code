...
import geotorch
from model._tlasso import run_glasso,em_tlasso,em_tlasso_noise_with_mu
...
def fit_ica_tlasso(Xs, ks, device, T, l,r, to_whiten, seed=1, params=None, scaling=True):
    ...
    def loss_func(W, Xl, Xd, tau1, tau2,n):
        loss = torch.trace(W.T@Xl@W@tau1) + torch.trace(W.T@Xd@W@tau2)
        #return torch.norm(torch.eye(n)-Xl@W@tau1@W-Xd@W@tau2@W, p='fro')
        return loss
    ...
    for i in range(num_views):
        ...
        n=X.shape[1]
        adj = pickle.load(open("/var/scratch/tpandeva/siica/data/adj_bs.pickle","rb"))
        n = X.shape[1]
    ...
    for t in range(T):   
        ...
        if L_t is None: 
            ...
            L_t, D_t, tauZ, tauN, muY = em_tlasso_noise_with_mu(S_all, Z_all, 1, l,r, L_t, D_t, glasso=True)
        ...
        for i in range(num_views): 
            ...
            for _ in range(2):
                ...
                Y1 = cp.Variable((n,n), "Y1",PSD=True)
                Y2 = cp.Variable((n,n), "Y2",PSD=True)
                ...
                def loss_closure():
                    ...
                    loss = torch.trace(D1_tensor@(ZQm.T@(L_t_tensor@ZQm))@D1_tensor)+torch.trace(D2_tensor@(ZQ.T@(D_t_tensor@ZQ))@D2_tensor)#
        ...
        L_t, D_t, tauZ, tauN, muY = em_tlasso_noise_with_mu(Z_np, N_np, 1, l,r, L_t, D_t)
        ...
        open_file = open(f"/var/scratch/tpandeva/siica/log/icatlasso/test_t_w_{l}.pickle", "wb")
        pickle.dump([Z_np,N_np, L_t, D_t, Xws, Tau, Ik, Ink, Wts], open_file)
        open_file.close()
...
def main(cfg) -> None:
    ...
    for s in range(cfg.start,cfg.end):
        ...
        open_file = open(f"{path}/res_{store_as_alias}_{l}_{cfg.start}_{cfg.end}.pickle", "wb")
